...
module.exports = async (client, guild) => {
  ...
  client.logger.info(`Celvox has joined ${guild.name}`);
  ...
  // Self-assign color
  try {
    const CelvoxColor = guild.roles.cache.find(r => r.name === '#Seagrass');
    if (CelvoxColor) await guild.me.roles.add(CelvoxColor);
  } catch (err) {
    client.logger.error(err.message);
  }
